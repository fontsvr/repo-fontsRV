...
def show_apk_warning(apk):
    ...
    xbmc.executebuiltin('Skin.SetString(apkinstaller, Ahora que {0} ha sido descargado[CR]Pulsa Instalar en la siguiente ventana)'.format(apk))
...
def show_build_prompt():
    class BuildPrompt(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.msg = "Actualmente {0} no esta instalada.\n\nSelecciona [B]'Menu Build'[/B] para instalarla o [COLOR FF12B2E7][B]'Ignorar'[/B][/COLOR] para no volver a ver este mensaje.\n\nGracias por elegir {1}.".format(CONFIG.ADDONTITLE, CONFIG.ADDONTITLE)
...
def show_update_window(name='Ventana de prueba', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    class UpdateWindow(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fanart = kwargs['fanart']
            self.msgupdate = "Actualizacion disponible para la build instalada:\n[COLOR {0}]{1}[/COLOR]\n\nVersion Actual: v[COLOR {2}]{3}[/COLOR]\nUltima Version: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*Recomendado: Instalacion Nueva[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
            self.msgcurrent = "Corriendo la ultima version para la build instalada:\n[COLOR {0}]{1}[/COLOR]\n\nVersion Actual: v[COLOR {2}]{3}[/COLOR]\nUltima Version: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*Recomendado: Instalacion Nueva[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
    ...
    # update.doModal()
    # del update
    msgcurrent = 'Corriendo la ultima version de la build instalada: '
    msgupdate = 'Actualizacion Disponible para la build instalada: '
    ...
    class LogViewer(xbmcgui.WindowXMLDialog):
        ...
        def onClick(self, controlId):
            if controlId == self.closebutton:
                ...
                if not newmsg:
                    self.setProperty('message.title', "Error Viendo Archivo de Registro")
                    self.setProperty('message.logmsg', "El archivo no existe o no se ha podido leer.")
                else:
                    self.logmsg = newmsg
